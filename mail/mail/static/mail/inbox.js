...
function view_email(event){
  ...
  let get_email = (email_id) => {
    ...
    .then(email => {
      ...
      document.querySelector('#reply').setAttribute('data-email_id', `${email.id}`);
  ...
  }else if (parseInt(element.parentElement.parentElement.parentElement.dataset.email_id)){

    get_email(parseInt(element.parentElement.parentElement.parentElement.dataset.email_id));
  ...
  if (mailbox === 'inbox' || mailbox === 'sent' || mailbox === 'archive'){
    ...
    .then(emails => {
      ...
      emails.forEach(email => {
        ...
        div.setAttribute('class', "d-flex align-items-center p-1 justify-content-between");

        const email_info = document.createElement("div");
        email_info.id = 'email-info-inbox';
        email_info.setAttribute('class', "d-flex flex-grow-1 bd-highlight align-items-center justify-content-between");
